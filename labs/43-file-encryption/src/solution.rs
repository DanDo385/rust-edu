...
// ## Classroom Narrative
//
// 1. **Memory safety**: We read files into `Vec<u8>` (heap allocated) and process them safely—no buffer overruns thanks to Rust's bounds checks.
// 2. **XOR cipher concept**: Each byte is XOR'd with the key. XOR is its own inverse, so the same function decrypts and encrypts. The key cycles via modulo arithmetic.
// 3. **File handling**: `encrypt_file_buffered` shows buffered I/O: we read 4KB chunks to limit heap pressure, process each chunk, and write it back.
//
// ## Symbol Drill
//
// - `&[u8]` parameters borrow slices of heap data without taking ownership—only when we call `fs::read` does Rust allocate the `Vec<u8>`.
// - `*` does not appear here (XOR uses `^`, not dereference).
//
// ## Step-by-step Teaching Breakdown
//
// 1. **Cipher primitive**: `xor_cipher` iterates over bytes, computing `byte ^ key[i % key.len()]`. The modulo keeps the key cycling without extra heap allocations.
// 2. **Full-file operations**: `encrypt_file`/`decrypt_file` call `fs::read`, producing owned `Vec<u8>` on the heap. We pass a borrow to `xor_encrypt` and write the resulting heap vec via `fs::write`.
// 3. **Buffered approach**: `encrypt_file_buffered` keeps a single 4KB buffer on the heap and reuses it. We read chunks, encrypt them, and immediately write, avoiding huge allocs for large files.
